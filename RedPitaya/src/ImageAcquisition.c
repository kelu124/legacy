...
#define IPhost "192.168.1.40"  //IP of the computeur for udp
	...
	//for (int k=0 ; k<500 ; k++){
		...
		if (PWM_Position!=oldPWM){
			...
			for (i=0 ; i<buff_size ; i++) {
				...
				/* Fin ecretage */
				ibuff[i] = (int)((1000*ttmp));  //integer value between -255 et 255
			}
			free(buff);
			/* End of cleaning*/
			...
			for (i=0 ; i < NbPackets ; i++) { // iteration sur l'ensemble des packets du buffer
				...
				UDPBuffer[1] = 0; // Number of the line
				UDPBuffer[2] = 0; //Number of the image
				UDPBuffer[3] = PWM_Position; //BUG @corriger
				UDPBuffer[4] = 0; //BUG @corriger à la reception -- je voulais mettre datetimestamp
				UDPBuffer[5] = 0;//(int)timediff(tStart, tTop); //BUG @corriger -- début de l'acquisition de la ligne
				UDPBuffer[6] = 0;//(int)timediff(tStart, tEnd); //BUG @corriger -- fin de l'acquisition de la ligne
